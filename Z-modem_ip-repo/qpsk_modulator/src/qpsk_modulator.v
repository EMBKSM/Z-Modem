module qpsk_modulator(
    ...
    always @(*) begin
        ...
        case (current_state)
            ...
            MODULATE: begin
                // Transition to UPDATE one cycle before the period ends to maintain exact timing
                // Period = 100. Count 0 to 98 (99 cycles) in MODULATE, then 1 cycle in UPDATE.
    ...
    always @(posedge clk or negedge reset) begin
        if (!reset) begin
            ...
            // Mixer Logic (Always running)
            i_mix <= i_val * dds_cos;
            q_mix <= q_val * dds_sin;
            tx_sum <= i_mix - q_mix;

            // PDM Logic (Always running)
